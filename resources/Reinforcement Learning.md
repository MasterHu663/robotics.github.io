...
<img src="../assets/images/DL-introduction.png" width="40%">
...
$$
a_t=\begin{cases}
\arg \max_{a\in A} \hat Q(a),\mathrm{with\ probability \ }1-\epsilon\\
\mathrm{random\ choice\ from\ A,\ } \mathrm{with\ probability\ }\epsilon
\end{cases}
$$
...
<img src="../assets/images/Markov_Decision_Process.png" width="40%">
...
(G_t = R_t + \gamma R_{t+1} + \gamma^2 R_{t+2} + \cdot \cdot \cdot= \sum ^{\infty}_{k=0}\gamma^kR_{t+k})\\
$$
...
$$
\nu^\pi(s)=(1-\gamma)\sum^\infty_{t=0}\gamma^tP_t^\pi(s)
$$
...
$$
\rho^\pi(s,a) = (1-\gamma)\sum^\infty_{t=0}\gamma^tP_t^\pi(s)\pi(a|s)
$$
...
选定任意的初始值$V^0$。可以得知$V^k=V^\pi$是以上更新公式的一个不动点，当$k\rightarrow \infty$时，序列${V^k}$会收敛到$V^\pi$，在实际的实现过程中，设定当某一轮的$\max_{s \in \mathcal S}\lvert V^{k+1}(s)-V^{k}(s)\lvert $的值非常小，提前结束迭代。
...
&= \mathbb E_\pi[\sum_{k=0}^{\infty}\gamma^kR_{t+k}|S_t=s] \\
&= \mathbb E_\pi[R_t+\gamma \sum_{k=0}^{\infty}\gamma^kR_{t+k+1}|S_t=s] \\
...
其中$\max_{a'}Q_{w^-}(s',a')$由目标网络（参数为$\omega^-$）计算得出，我们还可以将其写成如下形式：
...
<img src="../assets/images/psi.png" width="50%">
...
\begin{aligned}
J(\theta) &= \mathbb E_{s_0}[V^{\pi_\theta}(s_0)]\\
&= \mathbb E_{\pi_{\theta'}}[\sum^\infty_{t=0}\gamma^tV^{\pi_\theta}(s_t)-\sum^\infty_{t=1}\gamma^tV^{\pi_\theta}(s_t)]\\
&=-\mathbb E_{\pi_\theta'}[\sum^\infty_{t=0}\gamma^t(\gamma V^{\pi_\theta}(s_{t+1})-V^{\pi_\theta}(s_t))]
\end{aligned}
$$
...
&= \mathbb E_{\pi_\theta'}\left[\sum^\infty_{t=0}\gamma^t[r(s_t,a_t)+\gamma V^{\pi_\theta}(s_{t+1})-V^{\pi_\theta}(s_t)]\right]
\end{aligned}
$$
...
$$
\begin{aligned}
&=\mathbb E_{\pi_{\theta'}}\left[\sum^\infty_{t=0}\gamma^tA^{\pi_\theta}(s_t,a_t)\right] \\
&= \sum^\infty_{t=0}\gamma^t \mathbb E_{s_t\sim P^{\pi_{\theta'}}_t} \mathbb E_{a_t\sim \pi_{\theta'}(\cdot|s)}[A^{\pi_\theta}(s_t,a_t)]\\
...
<img src="../assets/images/beta.png" width="50%">
...
$$
在强化学习中，我们可以使用$H(\pi(\cdot\lvert s))$来表示策略$\pi$在状态$s$下的随机程度。
...
$$
\begin{aligned}
L_Q(\omega) &= \mathbb E_{\substack{(s_t,a_t,r_t,s_{t+1}) \sim R \\ a_{t+1}\sim\pi_\theta}} 
\left[ \frac{1}{2} \Big( Q_\omega(s_t,a_t) - \mathcal{T} \Big)^2 \right] \\
\text{ where } \mathcal{T} &= r_t + \gamma \Big( \min_{j=1,2} Q_{\omega_{j^-}}(s_{t+1},a_{t+1}) - \alpha \log \pi(a_{t+1}|s_{t+1}) \Big)
\end{aligned}
$$
...
可以理解为最大化函数$V$，因为有$V(s_t) = \mathbb E_{a_t \sim \pi}[Q(s_t,a_t)-\alpha \log \pi(a_t\lvert s_t)]$。
